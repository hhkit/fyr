...
\paragraph{Forward+} Forward rendering refers to performing the entire graphics pass, from geometry through to shading, in a single render pass. \cite{haradaForwardBringingDeferred2012} takes the tiling of the deferred shading pipeline and applied it to forward rendering, tiling the camera frustum and performing frustum culls of lights in the scene using a compute shader. The insight drawn from Forward+ was that deferred shading was not the final destination of graphics optimization; the true cost may be memory traffic. 
...
To begin research into making a compiler for the GPU, we must first delve into what the state of the art of compilers for the GPU are. After being introduced to MLIR's metainfrastructure by working on IRDL\cite{MLIRIRDLAddedIRDL,fehrIRDLIRDefinition2022}, we then started work on IREE in the latter half of 2025. The attention mechanism\cite{vaswaniAttentionAllYou2023} in large language models is a parallel algorithm that gives the transformer the ability to infer context.



This paper is still a work in progress, as we have not fully integrated the functionality into IREE for a proper benchmark. However, a demo is planned to be made at C4ML (deadline of December 2025) followed by a submission at OOPSLA in 2026.
