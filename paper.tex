...
The rasterization pipeline is takes the graphics card from primitive bufers through to \autoref{fig:rasterpipe}
...
\caption{The acceleration structure (AS) layout as described in Nvidia's DirectX 12 Raytracing tutorial\cite{DX12RaytracingTutorial}. The top-level acceleration structures (TLAS) instance the bottom-level acceleration structures (BLAS), not unlike how objects in a scene instance the same meshes. \label{fig:raytracepipe}}  
\end{figure}

\cite{RayTracingVulkan2020}
