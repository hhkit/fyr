...
The integral over the hemisphere is what allows for computing global illumination in a realistic manner. By nature of being an integral over a 2D manifold, it is also computationally intensive, yet must be performed for every fragment in the render target to achieve realistic lighting. In real-time workflows, radiance probes\cite{silvennoinenRealtimeGlobalIllumination2017} placed throughout the scene collect global illumination. Radiance cascades\cite{RadianceCascadesPaperOut_latexmk2RadianceCascadespdf} utilize TMUs to accelerate the interpolation of irradiance data from adjacent radiance probes.
...
\paragraph{Post-processing} While the typical graphics pass produces a realistic image of the scene, many applications of real-time graphics are typically geared towards entertainment such as video games. Artists may tune the final image, applying \emph{post processing effects} such as depth of field\cite{mcintoshEfficientlySimulatingBokeh2012} and tone mapping\cite{eilertsenRealtimeNoiseawareTone2015} to achieve a desired effect on the viewer. As each of these effects forms, massive performance gains can be made by \emph{fusing} adjacent passes into a single shader, avoiding the materialization of intermediate textures and minimizing the number of draw calls.
...
A number of DSLs have been devised, some specfically for computer graphics, others for more generic computation. Taichi\cite{huTaichiLanguageHighperformance2019} is a high-performance DSL written for Python that is designed for high performance sparse programs, a class of programs that includes raytraced rendering. Luisarenderer \cite{zhengLuisaRenderHighPerformanceRendering} and RenderKernel \cite{yangRenderKernelHighlevelProgramming2024} are two attempts at creating a high-level DSL for raster pipelines, attempting to use Python's decorator functionality to specify which pieces of code are kernels to be dispatched to the GPU.
...
To begin research into making a compiler for the GPU, we must first delve into what the state of the art of compilers in the GPU are. In the latter half of 2025, we have been working on a paper on tiling the attention mechanism\cite{vaswaniAttentionAllYou2023} in large language models, where we made the crucial insight that enabling online attention through the online softmax rewrite\cite{milakovOnlineNormalizerCalculation2018} opens the door to complete tiling of the attention operation.
...
RQ1 will be addressed by the raytracing paper, as we apply compiler techniques to not only apply the raytracing pipeline to sparse linear algebra, but fuse adjacent computations into the loop as well. Success will be determined by how our compiler-based solution holds up against the state of the art sparse linear algebra libraries such as cuSPARSE\cite{CuSPARSE} and MAGMA\cite{abdelfattahMAGMAEnablingExascale2024}.

To answer RQ2, the raster rewrite paper will only partially answer it. As rendering engines are immensely complex and configurable engines, there are far too many intricate details to handle. Hence, we will benchmark our prototype graphics engine against tutorial projects by LearnOpenGL\cite{vriesJoeyDeVriesLearnOpenGL2025} and Sascha Willems\cite{SaschaWillemsVulkanExamples}, which implement deferred shading and other techniques atomically to form a more stable control. If this first paper proves successful, implementing the advanced techniques in engines such as Unreal Engine 5's Lumen\cite{LumenGlobalIllumination} will allow us to verify the claim that program rewrites can yield an effective, maintainable codebase for a render engine with comparable performance to the state of the art.

\section {Timeplan}

The biggest issue pointed out by my supervisor is that of \emph{risk}. Compilers are exceedingly good at optimizing for low-level semantics, and linear algebra in particular very neatly lowers to perfectly nested loops which are perfect for optimizations such as vectorization and memory tiling. In comparison, a compiler for dealing with high-level constructs such as render pipelines may not yield as much.

Thus, this timeplan will prioritize the more generally applicable papers of attention and sparse matrix multiplication, all the while building a GPU-targeted dispatch dialect in the background, such that a pivot to a safer, more applicable research direction in the third year would not derail the course of the PhD.
